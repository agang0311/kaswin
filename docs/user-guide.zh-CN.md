...
点击 **Create round** 前，把鼠标放在按钮上可查看费用明细。创建会暂时锁定默认 50 KAS carrier，并使用 registry reserve 和交易费。carrier 不是奖池，正常 finalize 或 refund 后会扣除 covenant fee 并退回创建者。
