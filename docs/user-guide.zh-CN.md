...
- **Total tickets**：总票数，最多 1,000,000；仍然最多只能分成 20 个购买批次。
...
点击 **Create round** 前，把鼠标放在按钮上可查看完整费用明细。创建会暂时锁定默认 0.2 KAS carrier。carrier 不是奖池，新 v3.5 轮次 finalize 扣除 0.02 KAS 后默认退回 0.18 KAS；timeout refund 扣除 0.03 KAS 后默认退回 0.17 KAS。页面允许的最低 carrier 是 0.1 KAS。
