...
- 测试网默认随机数是开发测试方案；主网必须配置独立外部 Oracle，且当前项目仍未经独立安全审计。
...
点击 **Create round** 前，把鼠标放在按钮上可查看完整费用明细。创建会暂时锁定默认 0.2 KAS carrier。v4 finalize 扣除 0.022 KAS 后退回 0.178 KAS；timeout refund 每次从该票价扣除 0.019 KAS，默认向票主退回 0.281 KAS，最后一票同时退回 carrier。页面允许的最低 carrier 是 0.1 KAS。
...
- 创建后，默认 0.2 KAS carrier 会锁在 round covenant 中，结束时扣除链上 covenant fee 后退回。
