...
- Mainnet：`raffle-v12-chain-pow-mainnet`
- TN12：`raffle-v12-chain-pow-tn12`
...
每次购票都会更新同一个奖池 covenant UTXO 和深度 20 的票据 Merkle 树。售罄后，或达到可配置的超时时间后，任意持票钱包可以直接执行一笔 `Draw & Pay`：
...
超时后购票入口会拒绝继续延长票链。开奖者必须提供自己的票据 Merkle proof；授权 UTXO 在派奖交易中原额返回。

### 安全边界

目标区块由售罄交易的 covenant UTXO DAA，或预先写入合约的销售截止 DAA 唯一确定。执行 `Draw & Pay` 的钱包只能提交 witness；合约会重新计算区块哈希、验证选中父区块和 DAA 跨越、读取链上 sequencing commitment、重算中奖票，并强制奖池输出地址和金额。因此创建者、买家、indexer、RPC 节点和开奖触发者都不能选择随机种子、替换中奖票或重抽；恶意服务最多拒绝提供数据，任何人都可以换节点或 indexer 后继续。
...
- 超过 1000 张票：配置独立 `indexer/raffle-indexer.mjs`，由它保存票据树并返回中奖票和参与者 proof。
