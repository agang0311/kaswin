...
当前仓库版本为 **Kaswin 0.9.13.1**，适用且只允许新建/操作以下当前合约版本：
...
> v0.9.13.1 是 v0.9.13 的页面显示补丁，合约协议和 artifact hash 保持不变，并继续以预发布集成候选提供。当前 artifact 已完成本地自动门禁和一轮 Mainnet 创建、Registry、购票、售罄开奖闭环，但完整 Testnet A–E、Mainnet 退款闭环、KasWare/Kastle 与移动端 E2E、静态 HTTPS、干净环境复现和独立安全审计仍未全部完成。不要把预发布版本视为已审计的生产系统。
...
- `raffle-vnext-liveness-guard-b1000`：v0.9.13.1 当前页面包，合约 artifact 与 v0.9.13 相同，可创建和操作。
...
2026-07-20，当前 v0.9.13/v0.9.13.1 Round artifact 在 Mainnet 完成了小额售罄闭环：
