...
fn main() {
    ...
    // Build the production SEALED covenant script:
    ...
    let p_sp_ts_mal = p_concat[0..9].to_vec();
    let p_daa_mal = p_concat[9..17].to_vec();
    let p_blue_mal = p_concat[17..24].to_vec();
    ...
    sb_mal_p.add_data(&p_sp_ts_mal).unwrap();
    sb_mal_p.add_data(&p_daa_mal).unwrap();
    sb_mal_p.add_data(&p_blue_mal).unwrap();
    sb_mal_p.add_i64(ACTION_DRAW).unwrap();
    ...
    let target_sp_ts_mal = t_concat[0..7].to_vec();
    let target_daa_mal = t_concat[7..15].to_vec();
    let target_blue_mal = t_concat[15..24].to_vec();
    ...
    sb_mal_t.add_data(&target_sp_ts_mal).unwrap();
    sb_mal_t.add_data(&target_daa_mal).unwrap();
    sb_mal_t.add_data(&target_blue_mal).unwrap();
    ...
    sb_mal_t.add_i64(ACTION_DRAW).unwrap();
    ...
    println!("\n>>> ALL TESTS 1 THROUGH 7 PASSED WITH PRODUCTION SEALED COVENANT! <<<");
}
