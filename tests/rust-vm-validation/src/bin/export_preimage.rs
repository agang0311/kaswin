...
fn main() {
    ...
    let preimage = serialize_canonical_preimage(&header_valid);
    let hex = faster_hex::hex_string(&preimage);
    fs::write("/root/kaswin/artifacts/tn10/phase-c/header-preimage.hex", hex).unwrap();
    println!("Exported header-preimage.hex ({} bytes)", preimage.len());
}
